...
{
    public class KaggleData : IDataIO<float, int>, IDataIO<double, int>
    {
        double[,] IDataIO<double, int>.ReadTrainingData(string filePath, int startLine, int count, out int[] labels)
        {
        ...
        double[,] IDataIO<double, int>.ReadTestData(string filePath, int startLine, int count)
        {
            var ret = new double[count, 784];
            using (FileStream fs = File.OpenRead(filePath))
            using (var sr = new StreamReader(fs))
